...
import { CourseAdmin, CourseAdminUpdate, SubjectAdmin } from '@/types/definition';
...
function Page() {
    ...
    const submit = async (data: CourseAdminUpdate) => {
        const url = domainAdmin + `/subject/edit/${params.subjectId}`;
        ...
        try {
            ...
            if(res.status === 200 ) {
                return navigation.replace('/admin/subject');
            }
        } catch {
    ...
    useEffect(() => {
        const fetch = async () => {
            const url = domainAdmin + `/subject/${params.subjectId}`;
            const header: HeadersInit = {
                "authorization": getCookie('accessToken') || '',
                "x-client-email": user?.email || ''
            };
        ...
        <div>
            ...
            <form onSubmit={handleSubmit(submit)}>
                <div className='space-y-3'>
                    <InputAdmin<CourseAdminUpdate> label='name' name='name' type='text' register={register} defaultValue={subject?.name}/>
                </div>
