...
function Page() {
    ...
    const submit = async (data: LessonUpdate) => {
        const url = domainAdmin + `/lesson/create`;
        const header: HeadersInit = {
            "authorization": getCookie('accessToken')?.toString() || '',
            "x-client-email": user?.email || ''
        }
        data = {...data, chapter_id: parseInt(params.chapterId) }
        ...
        <div>
            ...
            <form onSubmit={handleSubmit(submit)} className='space-y-4'>
                <div className='space-y-3'>
                    <InputAdmin<LessonUpdate> label='title' name='title' type='text' register={register}/>
                    <InputAdmin<LessonUpdate> label='position' name='position' type='number' register={register}/>
                    <InputAdmin<LessonUpdate> label='video_link' name='video_link' type='text' register={register}/>
                </div>
